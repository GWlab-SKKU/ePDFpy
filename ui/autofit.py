...
def Kirkland_factor(cf, pix_min, pix_max):
    ...
    for j in np.arange(0, len(a1), 1):
        ...
        for i in np.arange(1, len(pix), 1):
            Kf[j ,i] = (a1[element])/(b1[element] + s2[i]) + (c1[element] * np.exp((-1) * d1[element] * s2[i])) + (a2[element])/(b2[element] + s2[i]) + (c2[element] * np.exp((-1) * d2[element] * s2[i])) + (a3[element])/(b3[element] + s2[i]) + (c3[element] * np.exp((-1) * d3[element] * s2[i]))
...
def Lobato_factor(cali, pix_min, pix_max):
    ...
    for j in np.arange(0, len(a1), 1):
        ...
        for i in np.arange(0, len(pix), 1):
            Lf[j, i] = (A1[element] * ((2 + (B1[element]* q2[i]))/(1 + (B1[element]* q2[i]))**2)) + (A2[element] * ((2 + (B2[element]* q2[i]))/(1 + (B2[element]* q2[i]))**2)) + (A3[element] * ((2 + (B3[element]* q2[i]))/(1 + (B3[element]* q2[i]))**2)) + (A4[element] * ((2 + (B4[element]* q2[i]))/(1 + (B4[element]* q2[i]))**2)) + (A5[element] * ((2 + (B5[element]* q2[i]))/(1 + (B5[element]* q2[i]))**2))
...
def Autofit_Kirkland(Iq, qkran_start, qkran_end, qkran_step ,pixran_start, pixran_end, pixran_step, Elem, Rat, pixel_start_n, Calibration_factor, Damping_factor, Noise_threshold, Select, use_lobato):
...
#         pixran = np.arange(1200,1050,-5)
    ...
    if len(qk_list) == 0 or len(pixran) == 0:
        return print('Please check the inputs')
    ...
    kf1, kf2, kf3, kf4, kf5 = Kf[Elem[0],:], Kf[Elem[1],:], Kf[Elem[2],:], Kf[Elem[3],:], Kf[Elem[4],:]  #Defining scattering factor for each elements
...
#        print(max_pix)
        for q_max in qk_list:
            ...
            ap = np.arange(int(pix_m), int(max_pix+1), 1)  ##Edited 2/21 -> Applied for 'newind' azav files. Slicing Min ~ Max pixels.
            
            temp_Q[:len(ap)], temp_q[:len(ap)], temp_i[:len(ap)], temp_k[:len(ap)], temp_k2[:len(ap)]= Q[ap], q[ap], intensity[ap], kf2_Q[ap], kf_2_Q[ap]
            ...
            fit_r = np.arange(int((2 * np.pi)/cali)-int(q_min/cali),int(q_max/cali)-int(q_min/cali) + 1,1)  #tail fitting from 2pi
...
####################### Filtering ########################################
    ...
    Results = pd.DataFrame(Autofit,columns=['Min pix','Max pix', 'qk', 'N' ,'Q','Phi(q)','Phi_d(q)','r','G(r)','judge1','judge2','judge3','judge4'])
    ...
    if qualitycheck != 0:                                    # For good samples which passed S.C 1
        if qualitycheck < Select:
            Candidates = Results[:qualitycheck].sort_values(['Max pix','qk','N'], ascending=[False,False,False]).to_numpy()
        if qualitycheck >= Select:
